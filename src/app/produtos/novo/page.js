...
export default function NovoProduto() {
  ...
  const [custoPacote, setCustoPacote] = useState(''); // O que ele gastou para comprar
  const [precoVenda, setPrecoVenda] = useState(''); // Por quanto vai vender a unidade
  ...
  const custoPorUnidade = (parseFloat(custoPacote) || 0) / (totalUnidades || 1);
  const faturamentoEsperado = (parseFloat(precoVenda) || 0) * totalUnidades;
  const lucroEsperado = faturamentoEsperado - (parseFloat(custoPacote) || 0);
    ...
    <div className="min-h-screen bg-gray-50 pb-24">
      ...
      <div className="p-6">
        <form onSubmit={salvar} className="bg-white p-5 rounded-2xl shadow-sm border space-y-4">
          ...
          <div>
            <label className="text-xs text-gray-500 font-bold">💰 Quanto você gastou nessa compra total? (R$)</label>
          ...
          <div>
            <label className="text-xs font-bold text-gray-800">🏷️ Por quanto vai vender 1 UNIDADE? (R$)</label>
          ...
          <div className="bg-emerald-50 p-4 rounded-xl border border-emerald-100 text-sm text-emerald-900 space-y-1">
            ...
            <p>💸 Custo por 1 unidade: <strong>R$ {custoPorUnidade.toFixed(2)}</strong></p>
